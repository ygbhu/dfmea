...
from quality_methods.dfmea.trace_service import DfmeaTraceResult, trace_causes, trace_effects
...
def trace_causes_command(
    ...
    fm: str | None = typer.Option(None, "--fm", "--failure-mode"),
    ...
    _run_trace_command(
        command_name="dfmea trace causes",
        workspace=workspace,
        project=project,
        ...
        action=lambda context: trace_causes(
            ...
            failure_mode_ref=_required_option(fm, "--fm"),
            depth=depth,
        ),
    )
...
def trace_effects_command(
    ...
    _run_trace_command(
        command_name="dfmea trace effects",
        workspace=workspace,
        project=project,
        output_format=output_format,
        quiet=quiet,
...
def _run_trace_command(
    *,
    ...
    quiet: bool,
    action: Callable[[DfmeaProjectContext], DfmeaTraceResult],
) -> NoReturn:
