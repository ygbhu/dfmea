...
from dfmea_cli.contracts import failure_result, validation_result
...
def validate_command(
    ...
    payload = validation_result(issues=report.issues, meta=meta)
    exit_code = (
        0 if payload["ok"] else CliError(code="VALIDATION_FAILED").resolved_exit_code
    )
    emit_payload(
        payload,
...
def _emit_failure(
    *,
    ...
    payload = failure_result(command="validate", errors=[error.to_error()], meta=meta)
    emit_payload(
        payload,
        output_format=output_format,
        quiet=False,
