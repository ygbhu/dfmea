...
>
> 文档状态：V1 架构基线（修订：CLI-first portable contract）
>
...
本架构用于支持一个可供任意 Agent 使用的 DFMEA skill 包。V1 继续采用本地 SQLite 数据库作为唯一事实来源，继续通过 Markdown 导出提供人类可读视图和 Git 审计能力；但对外的正式产品边界改为本地 `dfmea` CLI。Skill 仍然存在，但其职责从“直接操作存储”收敛为“理解任务并路由到 CLI 命令”。
...
系统采用“Agent 适配层 + CLI 接口层 + 领域服务层 + 存储层 + 导出与校验层”的五层架构。

```mermaid
graph TD
    USER["Human / Agent Task"] --> ADAPTER["Skill / Agent Adapter Layer"]
    ADAPTER --> CLI["dfmea CLI Interface Layer"]
    CLI --> SERVICE["Domain Service Layer"]
    SERVICE --> DB["SQLite Storage Layer\nSource of Truth"]
    SERVICE --> MAINTAIN["Validation / Export Layer"]
    MAINTAIN --> DB
    DB --> MARKDOWN["Markdown Export\nHuman Review / Git Audit"]
```
...
- 领域服务层：承载事务、ID 分配、层级校验、删除清理、失效链维护、递归追溯、校验编排和导出编排
- 存储层：使用 SQLite 持久化 `projects`、`nodes`、`fm_links`，并通过索引和递归 SQL 支撑查询能力
- 导出与校验层：生成 Markdown 导出，执行 `schema` / `graph` / `integrity` 校验，并向 CLI 返回可消费结果

关键边界：

- Skill 不直接写 SQLite，不把 Markdown 当源数据
- CLI 负责“可移植接口”，领域服务负责“业务规则执行”，SQLite 负责“事实持久化”
...
| 领域服务层 | 所有标准写规则都集中在共享 service core | 统一事务、ID 分配、删除清理、引用校验和导出/校验编排 |
...
| 数据库设计 | 3 张表：`projects` + `nodes` + `fm_links` | 极简设计，单表继承 + 跨层溯源专用链接表 |
...
| 查询能力形态 | `dfmea query` / `dfmea trace` 对外暴露，SQL 在内部支撑 | 对 Agent 暴露稳定命令能力，同时保留 SQL 性能和递归能力 |
...
| 类别 | 载体 | 说明 |
|------|------|------|
| 源数据 | SQLite `.db` 文件 | 唯一事实来源，所有标准写操作最终作用于此 |
...
```text
CLI command -> resolve project/node refs -> service transaction -> DB write(s) -> commit or rollback
```
...
- `dfmea validate`：执行 `schema`、`graph`、`integrity` 校验，并始终返回完整报告
- `dfmea export markdown`：生成 Markdown 派生视图，不改变源数据
...
- Agent 面向稳定命令能力编排任务
- 服务层保持统一解析、权限边界和错误语义
- SQL 优化可以在内部演进，而不要求所有 Skill 跟着改写查询实现

### 9.2 SQL 作为底层支撑而非对外契约

查询和递归追溯仍然由 SQL 支撑。下面的 SQL 片段描述的是内部实现能力和诊断能力，不是 Agent 的标准使用契约。
...
- `dfmea-query`：把查询、搜索、递归追溯路由到 `dfmea query *` 和 `dfmea trace *`
- `dfmea-maintenance`：把校验和导出路由到 `dfmea validate` 与 `dfmea export markdown`
...
| 大规模分析层管理 | SQL 索引和递归能力支撑 `query` / `trace` 命令，无需 Markdown 反查 |
...
| 数据可恢复 | SQLite 事务原子性 + WAL 恢复 + 定期备份 + Markdown 可重建 |
